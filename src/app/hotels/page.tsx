...
import { Star, MapPin, Wifi, Coffee, Utensils, Waves, Search, ChevronRight, Map as MapIcon, LayoutGrid } from "lucide-react";
...
export default function HotelsPage() {
  ...
  const filteredHotels = React.useMemo(() => {
    ...
    if (selectedState !== "All") {
      result = result.filter(h => h.state === selectedState || h.location?.toLowerCase().includes(selectedState.toLowerCase()));
    }
    if (searchQuery) {
    ...
    return result;
  }, [searchQuery, hotels, selectedState]);
                  ...
                  {paginatedHotels.map((hotel) => (
                    ...
                    >
                      <Link href={`/hotels/${hotel.id}`}>
                        <Card className={`group overflow-hidden border transition-all hover:shadow-lg ${hoveredId === hotel.id ? 'border-red-500 shadow-md ring-1 ring-red-500' : 'border-neutral-200 dark:border-neutral-800'}`}>
                          ...
                          <CardContent className="p-3">
                            <h3 className="text-sm font-bold truncate group-hover:text-red-600 transition-colors">{hotel.name}</h3>
                            <div className="flex items-center justify-between mt-1">
                              <p className="text-[10px] text-neutral-500 flex items-center gap-1">
                                <MapPin className="h-2 w-2" /> {hotel.location}
                              </p>
                              <span className="text-[10px] font-bold text-red-600">{hotel.price}</span>
                            </div>
                          </CardContent>
                        </Card>
                      </Link>
